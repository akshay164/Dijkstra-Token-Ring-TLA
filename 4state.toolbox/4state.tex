...
\@x{}\moduleLeftDash\@xx{ {\MODULE} 4state}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} Integers ,\, FiniteSets}%
\@x{ {\CONSTANT} N}%
\@x{ {\ASSUME} N \.{\in} Nat \.{\,\backslash\,} \{ 0 \}}%
\@x{ Procs \.{\defeq} 1 \.{\dotdot} N \.{-} 1}%
\@pvspace{16.0pt}%
\begin{lcom}{7.5}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Dijkstra\mbox{'}s stabilizing 4 state token ring with processes
\end{cpar}%
\end{lcom}%
\pcalsymbolstrue
\pcalshadingtrue
\csyntaxtrue
\@x{ {\p@mmalgorithm} TokenRing {\p@lbrace}}%
\@x{}%
\@y{\@s{2.5}%
 variable \ensuremath{c \.{=}} [\ensuremath{k \.{\in} 0\.{\dotdot}N
 \.{\mapsto}} (\ensuremath{k\.{\%}2})], \ensuremath{up \.{=}} [\ensuremath{k
 \.{\in} 0\.{\dotdot}N \.{\mapsto} {\IF}} \ensuremath{k\.{=}N}
 \ensuremath{\.{\THEN} {\FALSE}} \ensuremath{\.{\ELSE} {\TRUE}}];
}%
\@xx{}%
 \@x{\@s{16.4} {\p@variable} c \.{=} [ k \.{\in} 0 \.{\dotdot} N \.{\mapsto} 0
 ] ,\, up \.{=} [ k \.{\in} 0 \.{\dotdot} N \.{\mapsto} {\IF} k \.{=} 0
 \.{\THEN} {\TRUE} \.{\ELSE} {\FALSE} ] {\p@semicolon}}%
\@pvspace{8.0pt}%
 \@x{\@s{12.29} {\p@fair} {\p@process} {\p@lparen} j \.{\in} Procs
 {\p@rparen}}%
\@pvspace{8.0pt}%
 \@x{\@s{16.4} {\p@lbrace} J0\@s{.5}\textrm{:}\@s{3} {\p@while} {\p@lparen}
 {\TRUE} {\p@rparen}}%
\@x{\@s{34.18} {\p@lbrace} {\p@either}}%
 \@x{\@s{56.06} {\p@lbrace} {\p@await} c [ self ] \.{\neq} c [ ( self \.{-} 1
 ) ] {\p@semicolon}}%
\@x{\@s{77.95} c [ self ] \.{:=} c [ ( self \.{-} 1 ) ] {\p@semicolon}}%
\@x{\@s{77.95} up [ self ] \.{:=} {\TRUE} {\p@semicolon}}%
\@x{\@s{69.75} {\p@rbrace}}%
\@x{\@s{43.76} {\p@or}}%
 \@x{\@s{43.76} {\p@lbrace} {\p@await} c [ self ] \.{=} c [ ( self \.{+} 1 ) ]
 \.{\land} up [ self ] \.{=} {\TRUE} \.{\land} up [ ( self \.{+} 1 ) ] \.{=}
 {\FALSE} {\p@semicolon}}%
\@x{\@s{69.75} up [ self ] \.{:=} {\FALSE} {\p@semicolon}}%
\@x{\@s{61.55} {\p@rbrace}}%
\@x{\@s{43.76} {\p@rbrace}}%
\@x{\@s{16.4} {\p@rbrace}}%
 \@x{\@s{12.29} {\p@fair} {\p@process} {\p@lparen} i \.{\in} \{ 0 \}
 {\p@rparen}}%
 \@x{\@s{16.4} {\p@lbrace} I0\@s{.5}\textrm{:}\@s{3} {\p@while} {\p@lparen}
 {\TRUE} {\p@rparen}}%
 \@x{\@s{46.79} {\p@lbrace} {\p@await} ( c [ self ] \.{=} c [ 1 ] \.{\land} up
 [ 1 ] \.{=} {\FALSE} ) {\p@semicolon}}%
 \@x{\@s{56.38} c [ self ] \.{:=} ( c [ self ] \.{+} 1 ) \.{\%} 2
 {\p@semicolon}}%
\@x{\@s{46.79} {\p@rbrace}}%
\@x{\@s{16.4} {\p@rbrace}}%
 \@x{\@s{16.4} {\p@fair} {\p@process} {\p@lparen} k \.{\in} \{ N \}
 {\p@rparen}}%
\@pvspace{8.0pt}%
 \@x{\@s{16.4} {\p@lbrace} N0\@s{.5}\textrm{:}\@s{3} {\p@while} {\p@lparen}
 {\TRUE} {\p@rparen}}%
\@x{\@s{34.18}}%
\@y{%
 \ensuremath{up[self] \.{:=}
 {\FALSE}};\@s{45.0}\ensuremath{\.{\,\backslash\,}}* It is wrong to assign
 \mbox{'}up\mbox{'} value here, because what if program executes process in
 procs before process in \ensuremath{N} in the start
}%
\@xx{}%
 \@x{\@s{50.58} {\p@lbrace} {\p@await} c [ self ] \.{\neq} c [ ( self \.{-} 1
 ) ] {\p@semicolon}}%
\@x{\@s{60.16} c [ self ] \.{:=} c [ ( self \.{-} 1 ) ] {\p@semicolon}}%
\@x{\@s{50.58} {\p@rbrace}}%
\@x{\@s{16.4} {\p@rbrace}}%
\@x{ {\p@rbrace}}%
\@y{}%
...
\@x{ {\VARIABLES} c ,\, up}%
\@pvspace{8.0pt}%
\@x{ vars \.{\defeq} {\langle} c ,\, up {\rangle}}%
\@pvspace{8.0pt}%
\@x{ ProcSet \.{\defeq} ( Procs ) \.{\cup} ( \{ 0 \} ) \.{\cup} ( \{ N \} )}%
\@pvspace{8.0pt}%
\@x{ Init \.{\defeq}}%
\@y{\@s{0}%
 Global variables
...
\@xx{}%
\@x{\@s{35.70} \.{\land} c \.{=} [ k \.{\in} 0 \.{\dotdot} N \.{\mapsto} 0 ]}%
 \@x{\@s{35.70} \.{\land} up \.{=} [ k \.{\in} 0 \.{\dotdot} N \.{\mapsto}
 {\IF} k \.{=} 0 \.{\THEN} {\TRUE} \.{\ELSE} {\FALSE} ]}%
\@pvspace{8.0pt}%
 \@x{ j ( self )\@s{1.16} \.{\defeq} \.{\lor} \.{\land} c [ self ] \.{\neq} c
 [ ( self \.{-} 1 ) ]}%
 \@x{\@s{59.88} \.{\land} c \.{'} \.{=} [ c {\EXCEPT} {\bang} [ self ] \.{=} c
 [ ( self \.{-} 1 ) ] ]}%
 \@x{\@s{59.88} \.{\land} up \.{'} \.{=} [ up {\EXCEPT} {\bang} [ self ] \.{=}
 {\TRUE} ]}%
 \@x{\@s{48.77} \.{\lor} \.{\land} c [ self ] \.{=} c [ ( self \.{+} 1 ) ]
 \.{\land} up [ self ] \.{=} {\TRUE} \.{\land} up [ ( self \.{+} 1 ) ] \.{=}
 {\FALSE}}%
 \@x{\@s{59.88} \.{\land} up \.{'} \.{=} [ up {\EXCEPT} {\bang} [ self ] \.{=}
 {\FALSE} ]}%
\@x{\@s{59.88} \.{\land} c \.{'} \.{=} c}%
\@pvspace{8.0pt}%
 \@x{ i ( self )\@s{1.59} \.{\defeq} \.{\land} ( c [ self ] \.{=} c [ 1 ]
 \.{\land} up [ 1 ] \.{=} {\FALSE} )}%
 \@x{\@s{48.77} \.{\land} c \.{'} \.{=} [ c {\EXCEPT} {\bang} [ self ] \.{=} (
 c [ self ] \.{+} 1 ) \.{\%} 2 ]}%
\@x{\@s{48.77} \.{\land} up \.{'} \.{=} up}%
\@pvspace{8.0pt}%
 \@x{ k ( self ) \.{\defeq} \.{\land} c [ self ] \.{\neq} c [ ( self \.{-} 1 )
 ]}%
 \@x{\@s{48.77} \.{\land} c \.{'} \.{=} [ c {\EXCEPT} {\bang} [ self ] \.{=} c
 [ ( self \.{-} 1 ) ] ]}%
\@x{\@s{48.77} \.{\land} up \.{'} \.{=} up}%
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq} ( \E\, self \.{\in} Procs \.{:} j ( self ) )}%
 \@x{\@s{47.82} \.{\lor} ( \E\, self \.{\in} \{ 0 \}\@s{4.07} \.{:} i ( self )
 )}%
\@x{\@s{47.82} \.{\lor} ( \E\, self \.{\in} \{ N \} \.{:} k ( self ) )}%
\@pvspace{8.0pt}%
 \@x{ Spec\@s{1.46} \.{\defeq} \.{\land} Init \.{\land} {\Box} [ Next ]_{
 vars}}%
 \@x{\@s{39.83} \.{\land} \A\, self \.{\in} Procs \.{:} {\WF}_{ vars} ( j (
 self ) )}%
 \@x{\@s{39.83} \.{\land} \A\, self \.{\in} \{ 0 \}\@s{4.07} \.{:} {\WF}_{
 vars} ( i ( self ) )}%
 \@x{\@s{39.83} \.{\land} \A\, self \.{\in} \{ N \} \.{:} {\WF}_{ vars} ( k (
 self ) )}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@pvspace{8.0pt}%
 \@x{ Tokens \.{\defeq} Cardinality ( \{ x \.{\in} Procs \.{:} c [ x ]
 \.{\neq} c [ ( x \.{-} 1 ) ] \.{\lor} ( c [ x ] \.{=} c [ ( x \.{+} 1 ) ]
 \.{\land} up [ x ] \.{=} {\TRUE} \.{\land} up [ ( x \.{+} 1 ) ] \.{=}
 {\FALSE} ) \} )}%
 \@x{\@s{50.12} \.{+} {\IF} ( c [ 0 ] \.{=} c [ 1 ] ) \.{\land} up [ 1 ] \.{=}
 {\FALSE} \.{\THEN} 1 \.{\ELSE} 0}%
 \@x{\@s{50.12} \.{+} {\IF} c [ N ] \.{\neq} c [ ( N \.{-} 1 ) ] \.{\THEN} 1
 \.{\ELSE} 0}%
\@x{ InvProp \.{\defeq} Tokens \.{=} 1}%
\@x{ Stabilization \.{\defeq} {\Diamond} InvProp}%
\@x{ LowerBound \.{\defeq} Tokens \.{\geq} 1}%
\@x{ NotIncrease \.{\defeq} {\Box} [ Tokens \.{'} \.{\leq} Tokens ]_{ vars}}%
 \@x{ Decrease \.{\defeq} \A\, m \.{\in} 1 \.{\dotdot} N \.{+} 1 \.{:} {\Box}
 {\Diamond} ( Tokens \.{\leq} m )}%
\@x{ TypeOK \.{\defeq} \A\, x \.{\in} 0 \.{\dotdot} N \.{:} c [ x ] \.{<} 2}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
\@pvspace{8.0pt}%
\setboolean{shading}{false}
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Dijkstra\mbox{'}s stabilizing 4 State token ring algorithm.
 Made by
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Akshay \ensuremath{Kumar \.{-} 50169103
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Rohin \ensuremath{Mittal \.{-} 50168799
}%
\end{cpar}%
\end{lcom}%
\end{document}
